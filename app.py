...
cursor.execute("CREATE TABLE IF NOT EXISTS video_games (id INT PRIMARY KEY NOT NULL AUTO_INCREMENT, popularite INT, titre VARCHAR(255), genre VARCHAR(255), annee_sortie VARCHAR(50), note_utilisateur VARCHAR(100), nombre_vote VARCHAR(100), histoire TEXT)")
    ...
    if result[0] != 14682:
        ...
        for row in dfCsv:
            ...
            if row["nombre_vote"] == "":
                row["nombre_vote"] = 0
            cursor.execute(
                "INSERT INTO video_games (popularite, titre, genre, annee_sortie, note_utilisateur, nombre_vote, histoire) VALUES (%s, %s, %s, %s, %s, %s, %s)",
...
def create_game():
    ...
    value_placeholders = ", ".join(["%s" for _ in insert_fields])
    insert_values = [data.get(field) for field in insert_fields]

    try:
        # remplacer (...) avec vos champs et données
        cursor.execute(
            f"INSERT INTO video_games ({field_sql}) VALUES ({value_placeholders})", insert_values)
        db_config.commit()
...
def update_game(game_id):
    ...
    update_sql = ", ".join([f"{field} = %s" for field in update_fields])
    update_values = [data.get(field) for field in update_fields]
    ...
    try:
        # remplacer (...) avec vos champs et données
        cursor.execute(
            f"UPDATE video_games SET {update_sql} WHERE id = %s", (*update_values, game_id))
        db_config.commit()
